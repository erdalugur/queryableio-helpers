    ...
    try {
        if (e.status == 200) {
            ...
            throw {
                statusCode: e.status,
                url: e.url,
                statusText: handleError(e.status, error && error.originalError && error.originalError.info && error.originalError.info.message || "")
            };
        }
        else {
            throw { statusCode: e.status, url: e.url, statusText: handleError(e.statusCode,e.statusText) };
        }
    } catch (error) {
        throw error;
    }
...
export async function runQuery(params) {
    try {
        ...
        const endpoint = configuration.endpoint + "/query";
        const c = getCredentials();
        const options = {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                "authorization": typeof(c) === "object"  ? c.token : ""
            },
            method: "POST",
 ...
 */
export async function runLogin({ model, keyField, valueField, fields, joins, relaxMode = true }) {
    return await runQuery({
        action: "login",
        model,
        keyField,
        ...
        if (relaxMode && x.token) { setCredentials(e.token) };
        return x
    })
}
...
export function setCredentials(data, days) {
    ...
    window.localStorage.setItem("token", JSON.stringify({ name: "token", value: data, expiryDate: getTime() }))
}
